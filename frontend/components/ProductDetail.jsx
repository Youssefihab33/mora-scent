...
import { X, ShoppingBag, Star, ShieldCheck, Clock, MessageSquare, Send, User as UserIcon } from 'lucide-react';
const ProductDetail = ({ product, onClose, onAddToCart, lang, reviews, onAddReview, user, isReviewSystemActive }) => {
    ...
    return (<div className="fixed inset-0 z-[80] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-5xl bg-white rounded-[2.5rem] shadow-2xl overflow-hidden max-h-[95vh] flex flex-col md:flex-row animate-fade-in-up">
        {/* Close Button */}
        <button onClick={onClose} className="absolute top-6 right-6 z-10 p-2 bg-white/20 backdrop-blur-lg text-white rounded-full hover:bg-white hover:text-black transition-all">
          <X size={24}/>
        </button>
        ...
        <div className="w-full md:w-1/2 bg-neutral-100 flex flex-col p-4 md:p-8">
          <div className="flex-1 relative rounded-3xl overflow-hidden bg-white shadow-inner flex items-center justify-center">
            <img src={activeImage} alt={product.name} className="max-h-[500px] w-auto object-contain transition-all duration-500"/>
          </div>
          
          <div className="flex gap-3 mt-6 overflow-x-auto py-2 scrollbar-hide px-2">
            {gallery.map((img, idx) => (<button key={idx} onClick={() => setActiveImage(img)} className={`flex-shrink-0 w-20 h-20 rounded-xl overflow-hidden border-2 transition-all ${activeImage === img ? 'border-[#D4AF37] scale-110 shadow-lg' : 'border-white'}`}>
        ...
        <div className="w-full md:w-1/2 p-8 md:p-12 overflow-y-auto" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
          <div className="flex items-center gap-2 mb-4">
            ...
            {isReviewSystemActive && avgRating && (<div className="flex items-center gap-1 text-[#D4AF37]">
          ...
          <div className="flex gap-4">
            <button onClick={() => onAddToCart(product)} className="flex-1 bg-[#1a1a1a] text-white py-5 rounded-2xl font-bold flex items-center justify-center gap-3 hover:bg-[#D4AF37] hover:text-black transition-all shadow-xl transform active:scale-95 group">
          ...
          {/* Reviews Section */}
          {isReviewSystemActive && (<div className="mt-12 pt-12 border-t border-neutral-100">
              ...
              {/* Review Form */}
              {user ? (<form onSubmit={handleReviewSubmit} className="bg-neutral-50 p-6 rounded-3xl mb-10 border border-neutral-100">
                  <div className="flex items-center gap-4 mb-4">
                    <img src={user.picture} className="w-10 h-10 rounded-full border-2 border-[#D4AF37]" alt={user.name}/>
                    <div>
                      <div className="font-bold text-sm">{user.name}</div>
                      <div className="flex gap-1 mt-1">
                        {[1, 2, 3, 4, 5].map((star) => (<button key={star} type="button" onClick={() => setNewRating(star)} className={`transition-colors ${star <= newRating ? 'text-[#D4AF37]' : 'text-neutral-300'}`}>
                  ...
                  <div className="relative">
                    <textarea value={newComment} onChange={(e) => setNewComment(e.target.value)} placeholder={lang === 'ar' ? 'اكتب رأيك هنا...' : 'Write your review here...'} className="w-full p-4 bg-white border border-neutral-100 rounded-2xl outline-none focus:border-[#D4AF37] resize-none h-24 text-sm"/>
                    <button type="submit" className="absolute bottom-4 left-4 bg-[#1a1a1a] text-white p-2 rounded-xl hover:bg-[#D4AF37] hover:text-black transition-all">
                      <Send size={18}/>
                    </button>
                  </div>
                </form>) : (<div className="bg-neutral-50 p-6 rounded-3xl mb-10 border border-dashed border-neutral-200 text-center">
              ...
              <div className="space-y-6">
                {reviews.length === 0 ? (<p className="text-center text-neutral-400 text-sm py-10">
                    {lang === 'ar' ? 'لا توجد تقييمات بعد. كن أول من يقيم!' : 'No reviews yet. Be the first to review!'}
                  </p>) : (reviews.map((review) => (<div key={review.id} className="flex gap-4 p-4 bg-white rounded-2xl border border-neutral-50 shadow-sm">
