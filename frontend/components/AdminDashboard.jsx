...
import { TrendingUp, ShoppingBag, Plus, Trash2, ArrowLeft, Image as ImageIcon, Upload, X, ClipboardList, Clock, Truck, Edit3, Warehouse, AlertTriangle, Banknote, ListTree, Users, Ban, FileText, RotateCcw, CreditCard, Settings, Mail, MessageCircle, Globe, Percent, Shield, Cpu, Lock, ShieldAlert, History, Key, Download, Ticket, MapPin } from 'lucide-react';
const AdminDashboard = ({ orders, setOrders, products, setProducts, categories, setCategories, customers, setCustomers, onClose, lang, isReviewSystemActive, setIsReviewSystemActive, storeSettings, setStoreSettings, adminUsers, setAdminUsers, activityLogs, setActivityLogs, coupons, setCoupons, shippingCompanies, setShippingCompanies, shippingZones, setShippingZones }) => {
    ...
    const handleAddCategory = () => {
        if (!newCat.ar || !newCat.en)
            return;
        setCategories(prev => [...prev, newCat]);
        setNewCat({ ar: '', en: '' });
    };
    const removeCategory = (arName) => {
        if (arName === 'الكل')
            return;
        setCategories(prev => prev.filter(c => c.ar !== arName));
    };
    const handleFileUpload = (e, isMain) => {
    ...
    return (<div className="min-h-screen bg-[#fcfcfc]" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <div className="max-w-7xl mx-auto p-8">
        ...
        {activeTab === 'shipping' && (<div className="space-y-8 animate-fade-in-up">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="bg-white p-8 rounded-3xl border border-neutral-100 shadow-sm">
                ...
                <div className="space-y-4">
                  {shippingCompanies.map(company => (<div key={company.id} className="flex justify-between items-center p-4 bg-neutral-50 rounded-2xl border border-neutral-100">
                      <div>
                        <div className="font-bold">{company.name}</div>
                        <div className="text-xs text-neutral-400">{company.contact}</div>
                      </div>
                      <button onClick={() => setShippingCompanies(prev => prev.map(c => c.id === company.id ? { ...c, isActive: !c.isActive } : c))} className={`px-3 py-1 rounded-full text-[10px] font-bold ${company.isActive ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
              ...
              <div className="bg-white p-8 rounded-3xl border border-neutral-100 shadow-sm">
                ...
                <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2">
                  {shippingZones.map(zone => (<div key={zone.id} className="p-4 bg-neutral-50 rounded-2xl border border-neutral-100 flex justify-between items-center">
                      <div>
                      ...
                      <div className="flex items-center gap-4">
                        <div className="text-right">
                          <input type="number" className="w-20 p-1 text-xs border rounded text-center outline-none focus:border-[#D4AF37]" value={zone.rate} onChange={(e) => setShippingZones(prev => prev.map(z => z.id === zone.id ? { ...z, rate: Number(e.target.value) } : z))}/>
                          <div className="text-[10px] font-bold text-neutral-400 mt-1">{currency}</div>
                        </div>
                        <button onClick={() => setShippingZones(prev => prev.map(z => z.id === zone.id ? { ...z, isActive: !z.isActive } : z))} className={`w-10 h-5 rounded-full relative transition-all ${zone.isActive ? 'bg-[#D4AF37]' : 'bg-neutral-300'}`}>
        ...
        {activeTab === 'coupons' && (<div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-fade-in-up">
            <div className="bg-white p-6 rounded-3xl border border-neutral-100 shadow-sm h-fit">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input type="text" className="w-full p-3 bg-neutral-50 border rounded-xl outline-none focus:border-[#D4AF37] font-mono uppercase" value={newCoupon.code} onChange={e => setNewCoupon({ ...newCoupon, code: e.target.value.toUpperCase() })} placeholder="E.G. SUMMER20"/>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-xs font-bold text-neutral-400 block mb-1">{lang === 'ar' ? 'نوع الخصم' : 'Discount Type'}</label>
                    <select className="w-full p-3 bg-neutral-50 border rounded-xl outline-none focus:border-[#D4AF37]" value={newCoupon.discountType} onChange={e => setNewCoupon({ ...newCoupon, discountType: e.target.value })}>
                  ...
                  <div>
                    ...
                    <input type="number" className="w-full p-3 bg-neutral-50 border rounded-xl outline-none focus:border-[#D4AF37]" value={newCoupon.discountValue} onChange={e => setNewCoupon({ ...newCoupon, discountValue: Number(e.target.value) })}/>
                  </div>
                </div>
                <div>
                  <label className="text-xs font-bold text-neutral-400 block mb-1">{lang === 'ar' ? 'أقل قيمة للطلب' : 'Min Order Value'}</label>
                  <input type="number" className="w-full p-3 bg-neutral-50 border rounded-xl outline-none focus:border-[#D4AF37]" value={newCoupon.minOrderValue} onChange={e => setNewCoupon({ ...newCoupon, minOrderValue: Number(e.target.value) })}/>
                </div>
                <div>
                  <label className="text-xs font-bold text-neutral-400 block mb-1">{lang === 'ar' ? 'تاريخ الانتهاء' : 'Expiry Date'}</label>
                  <input type="date" className="w-full p-3 bg-neutral-50 border rounded-xl outline-none focus:border-[#D4AF37]" value={newCoupon.expiryDate} onChange={e => setNewCoupon({ ...newCoupon, expiryDate: e.target.value })}/>
                </div>
                <button onClick={() => {
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              <div className="grid grid-cols-1 gap-4">
                {coupons.map(coupon => (<div key={coupon.id} className="bg-white p-6 rounded-3xl border border-neutral-100 shadow-sm flex justify-between items-center group">
                    <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-6">
                      ...
                      <button onClick={() => setCoupons(prev => prev.filter(c => c.id !== coupon.id))} className="p-3 bg-red-50 text-red-500 rounded-xl opacity-0 group-hover:opacity-100 transition-all hover:bg-red-500 hover:text-white">
                        <Trash2 size={18}/>
                      </button>
                    </div>
                  </div>))}
                {coupons.length === 0 && (<div className="bg-white p-12 rounded-3xl border border-dashed border-neutral-200 text-center text-neutral-400">
        ...
        {activeTab === 'security' && (<div className="space-y-8 animate-fade-in-up">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              {/* Admin Accounts */}
              <div className="bg-white p-8 rounded-3xl border border-neutral-100 shadow-sm">
                <h3 className="text-xl font-bold mb-6 flex items-center gap-2">
                ...
                <div className="space-y-4">
                  {adminUsers.map(admin => (<div key={admin.id} className="flex justify-between items-center p-4 bg-neutral-50 rounded-2xl border border-neutral-100">
                      <div>
                        <div className="font-bold text-sm">{admin.name}</div>
                        <div className="text-[10px] text-neutral-400">{admin.email}</div>
                        <div className="mt-1">
                          <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${admin.role === 'super_admin' ? 'bg-purple-100 text-purple-600' :
                    admin.role === 'admin' ? 'bg-blue-100 text-blue-600' : 'bg-neutral-200 text-neutral-600'}`}>
                      ...
                      <div className="flex gap-2">
                        ...
                        {admin.role !== 'super_admin' && (<button onClick={() => setAdminUsers(prev => prev.filter(a => a.id !== admin.id))} className="p-2 text-neutral-400 hover:text-red-500">
                            <Trash2 size={16}/>
                          </button>)}
                      </div>
                    </div>))}
              ...
              <div className="bg-white p-8 rounded-3xl border border-neutral-100 shadow-sm">
                ...
                <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2">
                  {activityLogs.length === 0 ? (<div className="text-center py-10 text-neutral-400 text-sm italic">
                      {lang === 'ar' ? 'لا توجد سجلات حالياً' : 'No logs available'}
                    </div>) : (activityLogs.map(log => (<div key={log.id} className="p-3 bg-neutral-50 rounded-xl border border-neutral-100 text-xs">
        ...
        {activeTab === 'settings' && (<div className="space-y-8 animate-fade-in-up">
            <div className="bg-white p-8 rounded-3xl border border-neutral-100 shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <input type="text" className="w-full p-4 bg-neutral-50 rounded-2xl border border-neutral-100 outline-none focus:border-[#D4AF37]" value={storeSettings.name} onChange={e => setStoreSettings({ ...storeSettings, name: e.target.value })}/>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <input type="text" className="w-full p-4 bg-neutral-50 rounded-2xl border border-neutral-100 outline-none focus:border-[#D4AF37]" value={storeSettings.logo} onChange={e => setStoreSettings({ ...storeSettings, logo: e.target.value })}/>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <input type="email" className="w-full p-4 bg-neutral-50 rounded-2xl border border-neutral-100 outline-none focus:border-[#D4AF37]" value={storeSettings.email} onChange={e => setStoreSettings({ ...storeSettings, email: e.target.value })}/>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <input type="text" className="w-full p-4 bg-neutral-50 rounded-2xl border border-neutral-100 outline-none focus:border-[#D4AF37]" value={storeSettings.whatsapp} onChange={e => setStoreSettings({ ...storeSettings, whatsapp: e.target.value })}/>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <input type="text" className="w-full p-4 bg-neutral-50 rounded-2xl border border-neutral-100 outline-none focus:border-[#D4AF37]" value={storeSettings.currency} onChange={e => setStoreSettings({ ...storeSettings, currency: e.target.value })}/>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <select className="w-full p-4 bg-neutral-50 rounded-2xl border border-neutral-100 outline-none focus:border-[#D4AF37]" value={storeSettings.defaultLanguage} onChange={e => setStoreSettings({ ...storeSettings, defaultLanguage: e.target.value })}>
                ...
                <div className="space-y-2">
                  ...
                  <input type="number" className="w-full p-4 bg-neutral-50 rounded-2xl border border-neutral-100 outline-none focus:border-[#D4AF37]" value={storeSettings.taxPercentage} onChange={e => setStoreSettings({ ...storeSettings, taxPercentage: Number(e.target.value) })}/>
                </div>
              </div>
              ...
              <div className="mt-6 space-y-2">
                ...
                <textarea className="w-full p-4 bg-neutral-50 rounded-2xl border border-neutral-100 outline-none focus:border-[#D4AF37] h-32 resize-none" value={storeSettings.policy} onChange={e => setStoreSettings({ ...storeSettings, policy: e.target.value })}/>
              </div>
            </div>
            ...
            <div className="bg-white p-8 rounded-3xl border border-neutral-100 shadow-sm">
              ...
              <div className="flex items-center justify-between p-6 bg-neutral-50 rounded-2xl border border-neutral-100">
                ...
                <button onClick={() => setStoreSettings({ ...storeSettings, aiDevelopmentEnabled: !storeSettings.aiDevelopmentEnabled })} className={`w-16 h-8 rounded-full relative transition-all duration-300 ${storeSettings.aiDevelopmentEnabled ? 'bg-[#D4AF37]' : 'bg-neutral-300'}`}>
              ...
              {storeSettings.aiDevelopmentEnabled && (<div className="mt-6 p-6 bg-emerald-50 rounded-2xl border border-emerald-100 flex items-center gap-4 animate-pulse">
            ...
            <div className="bg-white p-8 rounded-3xl border border-neutral-100 shadow-sm">
              ...
              <div className="flex items-center justify-between p-6 bg-neutral-50 rounded-2xl border border-neutral-100">
                ...
                <button onClick={() => setIsReviewSystemActive(!isReviewSystemActive)} className={`w-16 h-8 rounded-full relative transition-all duration-300 ${isReviewSystemActive ? 'bg-[#D4AF37]' : 'bg-neutral-300'}`}>
        ...
        {activeTab === 'stats' && (<div className="space-y-8 animate-fade-in-up">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white p-8 rounded-[2.5rem] border border-neutral-100 shadow-sm">
                ...
                <div className="space-y-4">
                  {orders.slice(0, 5).map(o => (<div key={o.id} className="flex justify-between items-center p-4 bg-neutral-50 rounded-2xl border border-neutral-100">
                      <div>
                  ...
                  {orders.length === 0 && (<div className="text-center py-10 text-neutral-400 text-sm">
              ...
              <div className="bg-white p-8 rounded-[2.5rem] border border-neutral-100 shadow-sm">
                ...
                <div className="space-y-4">
                  {products.filter(p => p.stock <= 5).slice(0, 5).map(p => (<div key={p.id} className="flex justify-between items-center p-4 bg-red-50 rounded-2xl border border-red-100">
                      <div className="flex items-center gap-3">
                  ...
                  {products.filter(p => p.stock <= 5).length === 0 && (<div className="text-center py-10 text-neutral-400 text-sm">
        ...
        {activeTab === 'categories' && (<div className="bg-white p-8 rounded-3xl border border-neutral-100 shadow-sm animate-fade-in-up">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {categories.map(c => (<div key={c.en} className="p-4 bg-neutral-50 rounded-2xl flex justify-between items-center border border-neutral-100">
                  <span className="font-bold">{lang === 'ar' ? c.ar : c.en}</span>
                  {c.ar !== 'الكل' && (<button onClick={() => removeCategory(c.ar)} className="text-red-400 hover:text-red-600"><X size={18}/></button>)}
                </div>))}
            </div>
          </div>)}

        {activeTab === 'customers' && (<div className="bg-white rounded-3xl border border-neutral-100 shadow-sm overflow-hidden animate-fade-in-up">
            ...
            <table className="w-full text-right">
              ...
              <tbody className="divide-y">
                {customers.map(c => (<tr key={c.id} className={`hover:bg-neutral-50 ${c.isBlocked ? 'opacity-50 grayscale' : ''}`}>
                    ...
                    <td className="px-6 py-4">
                      <input type="text" placeholder="..." className="text-xs bg-transparent border-b border-transparent focus:border-[#D4AF37] outline-none w-full" value={c.notes} onChange={(e) => {
                ...
                }} className={`p-2 rounded-lg transition-all ${c.isBlocked ? 'bg-red-500 text-white' : 'bg-red-50 text-red-500 hover:bg-red-500 hover:text-white'}`} title={c.isBlocked ? (lang === 'ar' ? 'إلغاء الحظر' : 'Unblock') : (lang === 'ar' ? 'حظر' : 'Block')}>
                          <Ban size={16}/>
                        </button>
        ...
        {activeTab === 'orders' && (<div className="bg-white rounded-3xl border border-neutral-100 shadow-sm overflow-hidden">
             <table className="w-full text-right">
                <thead className="bg-neutral-50 text-neutral-400 text-xs font-bold uppercase">
                  <tr>
                ...
                <tbody className="divide-y">
                  {orders.map(o => (<tr key={o.id} className="hover:bg-neutral-50 cursor-pointer group" onClick={() => setSelectedOrder(o)}>
                      ...
                      <td className="px-6 py-4">
                        <span className={`px-3 py-1 rounded-full text-xs font-bold ${o.status === 'جديد' ? 'bg-blue-100 text-blue-600' :
                    o.status === 'قيد التجهيز' ? 'bg-yellow-100 text-yellow-600' :
                      ...
                      <td className="px-6 py-4" onClick={(e) => e.stopPropagation()}>
                        <select className="text-xs border rounded-lg p-1 outline-none focus:border-[#D4AF37]" value={o.status} onChange={(e) => {
        ...
        {/* Order Details Modal */}
        {selectedOrder && (<div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <div className="bg-white rounded-[2.5rem] w-full max-w-2xl overflow-hidden shadow-2xl animate-fade-in-up">
              ...
              <div className="p-8 max-h-[70vh] overflow-y-auto">
                <div className="grid grid-cols-2 gap-8 mb-8 pb-8 border-b border-neutral-100">
                  <div>
                    ...
                    {selectedOrder.status === 'تم الشحن' && (<div className="mt-4 p-3 bg-blue-50 rounded-xl border border-blue-100">
                  ...
                  <div className="text-right">
                    ...
                    {selectedOrder.shippingCost && (<div className="text-xs text-neutral-400 mt-1">
                        {lang === 'ar' ? 'شامل الشحن:' : 'Incl. Shipping:'} {selectedOrder.shippingCost} {currency}
                      </div>)}
                  </div>
                </div>

                {selectedOrder.status === 'قيد التجهيز' && (<div className="mb-8 p-6 bg-neutral-50 rounded-2xl border border-neutral-100">
                    <h4 className="text-xs font-bold text-neutral-400 uppercase mb-4">{lang === 'ar' ? 'تجهيز الشحنة' : 'Prepare Shipment'}</h4>
                    <div className="grid grid-cols-2 gap-4">
                      <select className="p-3 border rounded-xl outline-none focus:border-[#D4AF37] bg-white text-sm" onChange={(e) => {
                    ...
                    setOrders(prev => prev.map(o => o.id === selectedOrder.id ? { ...o, shippingCompany: company } : o));
                      ...
                      <input type="text" placeholder={lang === 'ar' ? 'رقم التتبع' : 'Tracking Number'} className="p-3 border rounded-xl outline-none focus:border-[#D4AF37] text-sm" onChange={(e) => {
                ...
                <div className="space-y-4">
                  {selectedOrder.items.map((item, idx) => (<div key={idx} className="flex justify-between items-center p-4 bg-neutral-50 rounded-2xl border border-neutral-100">
                      <div className="flex items-center gap-4">
                ...
                {selectedOrder.customer.notes && (<div className="mt-8 p-4 bg-yellow-50 rounded-2xl border border-yellow-100">
              ...
              <div className="p-6 bg-neutral-50 border-t border-neutral-100 flex justify-end gap-3">
                <button onClick={() => window.print()} className="px-6 py-3 bg-white border border-neutral-200 rounded-xl font-bold text-sm flex items-center gap-2 hover:bg-neutral-100 transition-all">
                  <FileText size={18}/> {lang === 'ar' ? 'طباعة الفاتورة' : 'Print Invoice'}
                </button>
                <button onClick={() => setSelectedOrder(null)} className="px-6 py-3 bg-[#1a1a1a] text-white rounded-xl font-bold text-sm hover:bg-black transition-all">
                  {lang === 'ar' ? 'إغلاق' : 'Close'}
                </button>
              </div>
            </div>
        ...
        {activeTab === 'products' && (<div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="bg-white p-6 rounded-3xl border border-neutral-100 shadow-sm h-fit sticky top-24">
              ...
              <div className="space-y-5">
                <div className="flex flex-col items-center justify-center border-2 border-dashed border-neutral-200 rounded-2xl p-4 hover:border-[#D4AF37] transition-all cursor-pointer bg-neutral-50 group" onClick={() => mainImageRef.current?.click()}>
                  {newProduct.image ? (<div className="relative w-full">
                ...
                <div className="flex gap-3 pt-2">
                  {editingId && (<button onClick={resetForm} className="flex-1 bg-neutral-100 text-neutral-600 py-4 rounded-xl font-bold hover:bg-neutral-200 transition-all">
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              <div className="grid grid-cols-1 gap-4">
                {products.map(p => (<div key={p.id} className="bg-white p-4 rounded-2xl flex justify-between items-center shadow-sm border border-neutral-50 hover:border-[#D4AF37]/30 transition-all group">
                    <div className="flex gap-4 items-center">
                      <div className="relative">
                        <img src={p.image} className="w-20 h-20 rounded-xl object-cover border border-neutral-100"/>
                        {p.stock <= 5 && (<div className="absolute -top-2 -right-2 bg-red-500 text-white text-[10px] font-bold px-2 py-1 rounded-full flex items-center gap-1 shadow-lg">
                ...
                }} className="p-3 bg-blue-50 text-blue-600 rounded-xl hover:bg-blue-600 hover:text-white transition-all shadow-sm" title={lang === 'ar' ? 'تعديل' : 'Edit'}>
                        <Edit3 size={18}/>
                      </button>
                      <button onClick={() => {
                ...
                }} className="p-3 bg-red-50 text-red-600 rounded-xl hover:bg-red-600 hover:text-white transition-all shadow-sm" title={lang === 'ar' ? 'حذف' : 'Delete'}>
                        <Trash2 size={18}/>
                      </button>
                    </div>
                  </div>))}
