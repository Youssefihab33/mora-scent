...
const ChatWidget = ({ products, lang, storeSettings }) => {
    ...
    return (<>
      <button onClick={() => setIsOpen(!isOpen)} className="fixed bottom-6 right-6 z-50 bg-[#D4AF37] text-black p-4 rounded-full shadow-2xl hover:bg-[#b5952f] transition-all transform hover:scale-110 active:scale-95">
        {isOpen ? <X size={28}/> : <MessageCircle size={28}/>}
      </button>

      {isOpen && (<div className="fixed bottom-24 right-6 z-50 w-80 md:w-96 bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col border border-neutral-100 h-[500px] animate-fade-in-up">
          <div className="bg-[#1a1a1a] text-white p-5 flex justify-between items-center">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-[#D4AF37] rounded-full flex items-center justify-center font-serif text-black font-bold uppercase overflow-hidden">
                {storeSettings.logo.startsWith('http') ? (<img src={storeSettings.logo} alt={storeSettings.name} className="w-full h-full object-cover"/>) : storeSettings.logo}
              </div>
              <div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-neutral-50" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
            {messages.map((msg, idx) => (<div key={idx} className={`flex ${msg.role === 'user' ? (lang === 'ar' ? 'justify-start' : 'justify-end') : (lang === 'ar' ? 'justify-end' : 'justify-start')}`}>
            ...
            {isLoading && (<div className={`flex ${lang === 'ar' ? 'justify-end' : 'justify-start'}`}>
          ...
          <div className="p-4 bg-white border-t border-neutral-100 flex gap-2" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
            <input type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSend()} placeholder={lang === 'ar' ? "اكتب استفسارك هنا..." : "Type your query here..."} className="flex-1 bg-neutral-100 rounded-full px-5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#D4AF37] border-none"/>
            <button onClick={handleSend} disabled={isLoading || !input.trim()} className="p-2.5 bg-[#D4AF37] text-black rounded-full hover:bg-[#b5952f] disabled:opacity-30 disabled:cursor-not-allowed transition-all">
