...
import { X, Banknote, CheckCircle, Smartphone, Phone, Ticket } from 'lucide-react';
...
const CheckoutModal = ({ lang, user, onClose, total, cart, onOrderSubmit, storeSettings, coupons, shippingZones }) => {
    ...
    if (isSuccess && lastOrder) {
        return (<div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
           ...
           {step === 1 && (<div className="space-y-6">
            ...
            {step === 3 && (<div className="space-y-6">
                ...
                <label className="flex flex-col p-4 border rounded-2xl cursor-pointer hover:bg-neutral-50">
                   ...
                   {info.paymentMethod === 'instapay' && (<div className="mt-4 p-4 bg-white border border-[#D4AF37]/20 rounded-xl space-y-3">
                ...
                <div className="bg-neutral-50 p-6 rounded-2xl border border-neutral-100">
                  ...
                  <div className="flex gap-2">
                    <input type="text" placeholder={lang === 'ar' ? 'أدخل الكود هنا' : 'Enter code here'} className="flex-1 p-3 border rounded-xl outline-none focus:border-[#D4AF37] uppercase font-mono" value={couponCode} onChange={e => setCouponCode(e.target.value)}/>
                    <button onClick={handleApplyCoupon} className="bg-[#1a1a1a] text-[#D4AF37] px-6 rounded-xl font-bold hover:bg-black transition-all">
                      {lang === 'ar' ? 'تطبيق' : 'Apply'}
                    </button>
                  </div>
                  {couponError && <p className="text-red-500 text-xs mt-2 font-bold">{couponError}</p>}
                  {appliedCoupon && (<div className="mt-3 flex justify-between items-center bg-emerald-50 p-3 rounded-xl border border-emerald-100">
